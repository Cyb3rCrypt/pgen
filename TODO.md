...
| Item | Notes |
|------|-------|
| Single-file layout | All code is in `src/main.rs` (~1 545 lines). Consider splitting into `cli.rs` (`Args`, `Config`), `gen.rs` (`pgen`, UUID, TypeID, ULID), and `main.rs` (`main`, `run_*`) |
